...
function WorkSection({ id, className }) {  
  ...
  return (
    ...
    >
      ...
      <div className='flex flex-grow items-center h-full text-sm sm:text-base'>
        <ul className='w-full divide-y'>
          <li className='flex py-2 text-[#FBFCF8] font-semibold'>
            <span className='w-1/2 '>Company / Project</span>
            <span className='w-1/2 pl-2'>Category</span>
          </li>    
            {projects.map(({ id, title, description, type, link, stack }, i) => (
              ...
              >
                ...
                {/* <div className='flex hover:pl-2 transition-all ease-in-out duration-[250ms]'> */}
                  <span className='w-1/2 pl-2'>{title}</span>
                  <span className='w-1/2 pl-4'>{type}</span>
      ...
      //   id={id} 
      //   className={`
      //     min-h-[100svh] h-full w-full pb-[calc(100dvh-100svh)] p-4 md:px-8 lg:px-20 pt-20 
      //     text-[#E6E8E3] bg-zinc-700 
      //     flex flex-col place-content-center ${className}
      //   `}
      // >
